...
class _FakeTaskScheduler(ts.TaskScheduler):
  ...
  def schedule(self):
    ...
    if self.task.exec_task.node_id in self._block_nodes:
...
class TaskManagerTest(tu.TfxTest):
  ...
  def test_queue_multiplexing(self, mock_publish):
    ...
    execq = tq.TaskQueue('exec-queue')
    cancelq = tq.TaskIdQueue('cancel-queue')

    # Enqueue some tasks.
    trainer_task = _test_task('Trainer', 'test-pipeline')
    execq.enqueue(trainer_task)
    cancelq.enqueue(tq.TaskId.from_task(trainer_task))
    ...
    with tm.TaskManager(
        ...
        process_all_queued_items_before_exit=True) as _:
      # Enqueue more tasks after task manager starts.
      transform_task = _test_task('Transform', 'test-pipeline')
      execq.enqueue(transform_task)
      evaluator_task = _test_task('Evaluator', 'test-pipeline')
      execq.enqueue(evaluator_task)
      cancelq.enqueue(tq.TaskId.from_task(transform_task))

    # Ensure that all exec and cancellation tasks were processed correctly.
    self.assertCountEqual([trainer_task, transform_task, evaluator_task],
                          collector.scheduled_tasks)
    self.assertCountEqual([trainer_task, transform_task],
                          collector.cancelled_tasks)
    mock_publish.assert_has_calls([
        mock.call(mock.ANY, pipeline, trainer_task, mock.ANY),
        mock.call(mock.ANY, pipeline, transform_task, mock.ANY),
        mock.call(mock.ANY, pipeline, evaluator_task, mock.ANY)
    ],
                                  any_order=True)
